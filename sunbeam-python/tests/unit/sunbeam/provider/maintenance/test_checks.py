...
class TestNoEphemeralDiskCheck:
    ...
    def test_run(self, mocker):
        ...
        instances = [[Mock(), Mock(), Mock()], [Mock(), Mock(), Mock()]]
        mocker.patch.object(
            checks, "get_admin_connection", return_value=mock_conn)
...
class TestNoInstanceOnNodeCheck:
    ...
    def test_run_failed(self, mock_conn, mock_get_admin_connection, mock_guests_on_hypervisor):
        node = "node1"
        instances = [Mock(), Mock()]
    ...
    def test_run_force(self, mock_conn, mock_get_admin_connection, mock_guests_on_hypervisor):
        node = "node1"
        instances = [Mock(), Mock()]
        instances[0].id = "inst-0"
        instances[1].id = "inst-1"
