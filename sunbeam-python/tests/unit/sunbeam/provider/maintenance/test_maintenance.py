...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            (["node-1", "node-2", "node-3"], ["node-2", "node-3"], ["node-1", "node-3"]),
        ),
    ]
)
...
def test_get_node_roles(
    ...
    result = _get_node_roles(
        mock_deployment, mock_jhelper, mock_console, False, nodes)

    mock_run_plan.assert_called_with([step(mock_deployment, mock_jhelper)], mock_console, False)
...
def test_enable_maintenance(
    ...
    mock_run_watcher_host_maintenance_step.assert_has_calls(
        [
            call(jhelper=mock_jhelper.return_value, node=node)
    ...
    mock_checks.NodeisNotControlRoleCheck.assert_called_with(
        nodes=[], force=False)
...
def test_disable_maintenance(
    ...
    steps = [mock_enable_hypervisor_step, mock_run_watcher_host_maintenance_step]
    side_effects = [[Mock() for _ in compute_nodes] for _ in steps]
    for step, side_effect in zip(steps, side_effects):
        step.side_effect = side_effect
    ...
    mock_run_watcher_host_maintenance_step.assert_has_calls(
        [
            call(jhelper=mock_jhelper.return_value)
            for _ in compute_nodes
        ],
        any_order=True,
    )
