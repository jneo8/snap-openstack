...
class TestAddLDAPDomainStep:
    ...
    def test_is_skip(self):
        step = AddLDAPDomainStep(self.jhelper, self.feature, {})
        result = step.is_skip()
    ...
    def test_enable_first_domain(self, read_config, update_config, snap):
        ...
        step = AddLDAPDomainStep(self.jhelper, self.feature, self.charm_config)
        result = step.run()
    ...
    def test_enable_second_domain(self, read_config, update_config, snap):
        ...
        step = AddLDAPDomainStep(self.jhelper, self.feature, {"domain-name": "dom2"})
        result = step.run()
        step.tfhelper.write_tfvars.assert_called_with(
            {
                "ldap-apps": {
...
class TestDisableLDAPDomainStep:
    ...
    def test_is_skip(self):
        step = DisableLDAPDomainStep(self.jhelper, self.feature, "dom1")
        result = step.is_skip()
        assert result.result_type == ResultType.COMPLETED
    ...
    def test_disable_wrong_domain(self, read_config, update_config, snap):
        ...
        step = DisableLDAPDomainStep(self.jhelper, self.feature, "dom2")
        result = step.run()
        assert result.result_type == ResultType.FAILED
...
class TestUpdateLDAPDomainStep:
    ...
    def test_is_skip(self):
        step = UpdateLDAPDomainStep(self.jhelper, self.feature, self.charm_config)
        result = step.is_skip()
        assert result.result_type == ResultType.COMPLETED
    ...
    def test_update_wrong_domain(self, read_config, update_config, snap):
        ...
        step = UpdateLDAPDomainStep(self.jhelper, self.feature, {"domain-name": "dom2"})
        result = step.run()
        assert result.result_type == ResultType.FAILED
        assert result.message == "Domain not found"
